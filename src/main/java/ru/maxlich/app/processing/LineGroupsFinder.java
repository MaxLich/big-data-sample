...
public class LineGroupsFinder {
    ...
    private void checkGroups(TreeSet<Integer> groupsWithSameElems, int groupNumber) {
        for (int matchedGrNum : groupsWithSameElems) { //перебираем все группы с таким же элементом
            if (matchedGrNum != groupNumber) {
                unitedGroupStorage.put(matchedGrNum, groupNumber); //сохраняем инф-цию об объединённых группах
  ...
  /*  private void walkLineElements(String[] lineElements, TreeSet<Integer> groupsWithSameElems, List<LineElement> newElements) {
